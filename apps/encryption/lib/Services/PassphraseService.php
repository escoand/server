...
class PassphraseService {
	 ...
	 */
	public function setPassphraseForUser(string $uid, string $password, ?string $recoveryPassword = null): bool {
		// if we are in the process to resetting a user password, we have nothing
		// to do here
		if (isset(self::$passwordResetUsers[$uid])) {
			return true;
		}
		...
		if ($user && $uid === $user->getUID()) {
			...
			$encryptedPrivateKey = $this->crypt->encryptPrivateKey($privateKey, $password, $uid);

			// Save private key
			if ($encryptedPrivateKey !== false) {
				$key = $this->crypt->generateHeader() . $encryptedPrivateKey;
				$this->keyManager->setPrivateKey($user->getUID(), $key);
				return true;
			}
	 ...
	 */
	private function initMountPoints(string $userId): void {
